...
namespace Authorization.Application.Domain.Handler.Authorization
{
	public class PostLoginHandler
	{
		...
		public PostLoginHandler( IRepository<User> repository, HashPassword hasher, AuthOptions authOptions )
		{
		...
		{
			if ( CheckOnIdentity( request.Identificator, request.Password, out User user ) )
			{				
